...
async function downloadFile(itemPath, item: Client.ListingElement, targetDir: string): Promise<void> {
    const fileDownloadDefer: Q.Deferred<void> = Q.defer<void>();
    ...
    fileFTP.on("ready", () => {
        fileFTP.get(itemPath, (error, stream: NodeJS.ReadableStream) => {
            try {
                if (error) {
                    console.error(`Unable to download '${itemPath}', error: ${error.message}`);
                    failTask(`Unable to download '${itemPath}', error: ${error.message}`);
                } else {
                    ...
                    for (const item of listing) {
                        try {
                            ...
                            if (item.type === "d") {
                                ...
                                await downloadFromFTP(ftp, itemPath, path.join(targetDir, item.name));
                            } else if (item.type === "-") {
                                await downloadFile(itemPath, item, targetDir);
                            } else {
                                console.log(`Unsupported FTP file type '${item.type}' for: ${itemPath}`);
                            }
                        } catch (err) {
